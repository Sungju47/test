...
def load_df(path_or_buffer) -> pd.DataFrame:
    df = pd.read_csv(path_or_buffer)
    # win 컬럼 정리
    df["win_clean"] = df["win"].apply(lambda x: 1 if str(x).strip().lower() in ("1","true","t","yes") else 0) if "win" in df.columns else 0
    # 아이템/룬 문자열 정리
    for col in [c for c in df.columns if c.startswith("item")]:
...
if uploaded is not None:
    ...
    st.error("CSV를 찾을 수 없습니다. 업로드해주세요.")
    st.stop()
...
def item_stats(sub: pd.DataFrame) -> pd.DataFrame:
    item_cols = [c for c in sub.columns if c.startswith("item") and c.endswith("_id")]
    rec = []
    for c in item_cols:
...
def rune_stats(sub: pd.DataFrame) -> pd.DataFrame:
...
items_df = item_stats(dfc)
runes_df = rune_stats(dfc)

# ---------- 추천 아이템/룬 ----------
st.subheader(f"{sel_champ} 추천 아이템/룬")
c1, c2 = st.columns(2)

if not items_df.empty:
    top_item = items_df.iloc[0]
    item_id = top_item["item"]
    # Data Dragon CDN 기준 URL
    img_url = f"https://ddragon.leagueoflegends.com/cdn/13.20.1/img/item/{item_id}.png"
    c1.image(img_url, width=64)
    c1.caption(f"{item_id}\n픽수: {top_item['total_picks']} / 승률: {top_item['win_rate']}%")
else:
    c1.info("추천 아이템 없음")

if not runes_df.empty:
    top_rune = runes_df.iloc[0]
    # 룬은 이미지 id를 바로 쓰기 어려우므로 이름만 표시
    c2.markdown(f"**{top_rune['rune_core']} + {top_rune['rune_sub']}**")
    c2.caption(f"픽수: {top_rune['games']} / 승률: {top_rune['win_rate']}%")
else:
    c2.info("추천 룬 없음")
